...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn layui-btn-lg"><a  href="${ctx}/dingdan/add" >新增</a></button>
			...
			<tbody>
				<c:forEach items="${requestScope.list}" var="dept" varStatus="stat">
					<tr>
						...
						<td>${dept.name }</td>
						<td>${dept.phone }</td>
						<td>${dept.numberofglasses }</td>
						<td>${dept.lensnumber }</td>
						<td>${dept.money }</td>
						<%-- <td>${dept.pay }</td> --%>
						<%-- <td>${dept.balance }</td> --%>
						<td>${dept.yuan }</td>
              ...
              <span class="layui-btn layui-btn-normal layui-btn-mini">已启用</span></td> -->
                ...
                <i class="layui-icon">&#xe601;</i>
									...
									<a title="编辑" href="${ctx}/dingdan/add?id=${dept.id }"> <i
										class="layui-icon">&#xe642;</i>
							...
							<td>
							<c:choose>
								<c:when test="${dept.operations == 0 }">审核未通过</c:when>
